 ...
 * server checks the reader cookie, an ?invite= link token, or an admin
 * session there — and render it through the same client MDX pipeline the
 * editor's preview uses, so an unlocked page looks exactly like a published
 * one. No cookie and no invite → the PIN form.
 *
 * Arriving through an invite link (?invite=…) also redeems the token into a
 * per-page cookie, so refreshes and in-site navigation stay unlocked after
...
export function ProtectedContent({ slug }: { slug: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (invite) {
        // Redeem the invite into a per-page cookie (fire-and-forget — the
        // content request below also accepts the raw token).
        void fetch('/api/reader/unlock', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ invite, slug }),
        }).catch(() => {});
      }
      const qs = new URLSearchParams({ slug });
      if (invite) qs.set('invite', invite);
      const res = await fetch(`/api/reader/content?${qs.toString()}`);
      if (res.ok) {
  ...
  return (
    ...
    >
      ...
      {inviteDead && (
        <p style={{ margin: 0, maxWidth: 380, fontSize: 13, color: 'var(--color-fd-error, #dc2626)' }}>
          This invite link is invalid or has expired — ask whoever shared it for a fresh one.
        </p>
      )}
